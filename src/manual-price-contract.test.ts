...
describe('manual price persistence contract',()=>{
 ...
 it('awaits a database refetch and warns without encouraging duplicate input',()=>{expect(admin).toContain('const fresh=await load()');expect(admin).toContain('fresh.find(row=>row.price_version_id===result.priceVersionId)');expect(admin).toContain('중복 입력하지 말고 최신 데이터 조회를 눌러주세요.')})
