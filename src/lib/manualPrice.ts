...
import{CURRENT_PRICES_SELECT,fromCurrentPriceView,type CurrentPriceViewRow}from'./currentPrices'
...
export async function saveManualPrice(form:PriceRow):Promise<SavedManualPrice>{
 ...
 const[{data:version,error:versionError},{data:current,error:currentError}]=await Promise.all([
  ...
  supabase.from('current_prices').select(CURRENT_PRICES_SELECT).eq('price_version_id',priceVersionId).single(),
 ])
 ...
 if(currentError||!current)throw new Error(`가격은 저장되었지만 목록을 다시 불러오지 못했습니다. 중복 입력하지 말고 최신 데이터 조회를 눌러주세요. (current_prices 확인: ${currentError?.message||'현재 가격에서 저장 항목을 찾을 수 없습니다.'})`)
 return{row:{...fromCurrentPriceView(current as CurrentPriceViewRow),source_type:'KEY_IN'},priceVersionId,unchanged:before?.price_version_id===priceVersionId&&Number(before?.bulk_fob)===Number(form.bulk_fob)}
}
