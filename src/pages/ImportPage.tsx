...
export function ImportPage() {
  ...
  return <main className="container admin">
    ...
    {rows.length > 0 && <>
      ...
      <div className="import-top-scroll" ref={topScroll} onScroll={event => { if (tableScroll.current) tableScroll.current.scrollLeft = event.currentTarget.scrollLeft }} aria-hidden="true"><div /></div>
      <div className="table-wrap import-table" ref={tableScroll} onScroll={event => { if (topScroll.current) topScroll.current.scrollLeft = event.currentTarget.scrollLeft }}><table>
        <colgroup><col className="col-select" /><col className="col-status" /><col className="col-source" /><col className="col-style" /><col className="col-season" /><col className="col-item" /><col className="col-group" /><col className="col-option" /><col className="col-price" /><col className="col-price" /></colgroup>
        <thead><tr><th>Select</th><th>Status</th><th>Sheet<br />Row</th><th>Style</th><th>Season</th><th>Item<br />Type</th><th>Product<br />Group</th><th>Price<br />Option</th><th>Current<br />Bulk FOB</th><th>Incoming<br />Bulk FOB</th></tr></thead>
        <tbody>{visible.map(row => <tr className="import-row-group" key={row.key}>
          <td colSpan={10}>
            <table className="import-row-table"><colgroup><col className="col-select" /><col className="col-status" /><col className="col-source" /><col className="col-style" /><col className="col-season" /><col className="col-item" /><col className="col-group" /><col className="col-option" /><col className="col-price" /><col className="col-price" /></colgroup><tbody><tr className="import-main-row"><td><input className="import-checkbox" aria-label={`Import ${row.display_name} ${row.price_option}`} type="checkbox" checked={row.import} disabled={!selectable(row)} onChange={event => change(row.key, { import: event.target.checked })} /></td><td><span className={`status s-${row.status.replace(' ', '-')}`}>{row.status}</span></td><td className="source-cell">{row.sheet}<br /><b>{row.row}</b></td><td><textarea className="style-input" rows={2} value={row.display_name} onChange={event => change(row.key, { display_name: event.target.value })} /></td><td className="compact-cell">{row.season}</td><td><select value={row.item_type} onChange={event => change(row.key, { item_type: event.target.value as ItemType })}>{['PANT', 'JERSEY', 'GLOVE', 'OTHER'].map(value => <option key={value}>{value}</option>)}</select></td><td><input value={row.product_group} onChange={event => change(row.key, { product_group: event.target.value })} /></td><td><input value={row.price_option} onChange={event => change(row.key, { price_option: event.target.value })} /></td><td className="price-cell">{formatPrice(row.current_bulk_fob)}</td><td><input className="price-input" type="number" step=".0001" value={row.bulk_fob ?? ''} onChange={event => change(row.key, { bulk_fob: event.target.value ? Number(event.target.value) : null })} /></td></tr></tbody></table>
            <div className="import-row-details">
              <div><span>Difference</span><strong className={row.current_bulk_fob != null && row.bulk_fob != null && row.bulk_fob - row.current_bulk_fob > 0 ? 'up' : 'down'}>{row.current_bulk_fob != null && row.bulk_fob != null ? `${row.bulk_fob - row.current_bulk_fob >= 0 ? '+' : ''}${formatPrice(row.bulk_fob - row.current_bulk_fob)}` : '—'}</strong></div>
              <div><span>Current Updated Date</span><strong>{row.current_updated_at ? new Date(row.current_updated_at).toLocaleDateString() : '—'}</strong></div>
              <label>Change Reason<select value={row.change_reason} onChange={event => change(row.key, { change_reason: event.target.value as ChangeReason })} disabled={row.status !== 'Update'}><option value="">선택</option><option value="TYPO">오타 수정</option><option value="MATERIAL">자재단가 변경</option><option value="LABOR">공임 변경</option><option value="SPECIFICATION">사양 변경</option><option value="OTHER">기타</option></select></label>
              <label>Internal Remark<input value={row.internal_remark} onChange={event => change(row.key, { internal_remark: event.target.value })} /></label>
              <label>Public Remark (English)<input lang="en" value={row.public_remark_en} onChange={event => change(row.key, { public_remark_en: event.target.value })} /></label>
              <div className="row-message"><span>Message</span><strong>{row.message || '—'}</strong></div>
            </div>
          </td>
        </tr>)}</tbody>
      </table></div>
